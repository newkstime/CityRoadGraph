...
class Graph:    
    ...
    def get_edge(self, from_vert, to_vert): 
        ...
        """
        adj_list = self.neighbors_dict[from_vert.get_name()]
    ...
    def get_degree(self, vertex):
        ...
        """
        return len(self.neighbors_dict[vertex.get_name()])
    ...
    def df_search(self, vertex):
        ...
        return GraphTree(vertex, search_order, parents)
    ...
    def dfs(self, vertex, parents, search_order, has_visited):
        ...
        index = self.vert_dict[vertex.get_name()] 
        ...
        for edge in adj_list:
            ...
            index = self.vert_dict[neighbor.get_name()]
    ...
    def bf_search(self, vertex):
        ...
        while not bfs_queue.is_empty():
            ...
            for edge in self.neighbors_dict[vertex.get_name()]:
            
                # Use the queue to go broader when the other
                # end of the edge has not yet been visited
                index = self.vert_dict[edge.to_vertex.get_name()]
                if not has_visited[index]:
    ...
    def get_min_span_tree(self):
        """
        Return MST starting with vertex with 0 index 
        """
        return self.get_MST(self.vertices[0])

    def get_MST(self, starting_vert):
        """
        ...
        index = self.vert_dict[starting_vert.get_name()]      
        cost[index] = 0 
        
        # Total weight of the MST thus far      
        total_weight = 0             
        ...
        while len(search_order) < self.get_size():
            ...
            for index in range(self.get_size()):
               if (self.vertices[index] not in search_order and
                            (cost[index] < current_min_cost)):
                    current_min_cost = cost[index]
            ...
            name = self.vertices[min_cost_index].get_name()
            adj_list = self.neighbors_dict[name]
            for edge in adj_list:
                index = self.vert_dict[edge.to_vertex.get_name()]
        ...
        return MST(starting_vert, search_order, parents, total_weight)
    ...
    def get_shortest_path(self, source_vertex):
        ...
        index = self.vert_dict[source_vertex.get_name()]      
        cost[index] = 0
        ...
        return ShortestPathTree(source_vertex, search_order, parents, cost, self.vertices)
